...
class Laboratory {
  ...
  static async isTimeSlotAvailable(labId, appointmentDate) {
    console.log("Laboratory.isTimeSlotAvailable called with:", {
      labId,
      appointmentDate,
    });
      ...
      const normalized =
        typeof appointmentDate === "string"
          ? appointmentDate.includes("T")
            ? appointmentDate.replace("T", " ")
            : appointmentDate
          : appointmentDate;
      const mysqlDateTime = /\d{2}:\d{2}$/.test(normalized)
        ? `${normalized}:00`
        : normalized;
  ...
  static async isTimeSlotAvailableForDisplay(labId, slotLocal) {
    try {
      ...
      const normalized =
        typeof slotLocal === "string"
          ? slotLocal.includes("T")
            ? slotLocal.replace("T", " ")
            : slotLocal
          : slotLocal;
      const mysqlDateTime = /\d{2}:\d{2}$/.test(normalized)
    ...
    for (let hour = startHour; hour < endHour; hour++) {
      for (let minute = 0; minute < 60; minute += 30) {
        allPossibleSlots.push(
          `${date} ${hour.toString().padStart(2, "0")}:${minute.toString().padStart(2, "0")}:00`,
        );
      }
    }
