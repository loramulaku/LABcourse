...
function requireLab(req, res, next) {
    if (!req.user || req.user.role !== 'lab') return res.status(403).json({ error: 'Lab access required' });
    next();
}
...
router.get('/dashboard/history', authenticateToken, requireLab, async (req, res) => {
    try {
...
router.get('/dashboard/appointments', authenticateToken, requireLab, async (req, res) => {
    try {
        const labId = await getCurrentLabId(req.user.id);
        if (!labId) return res.json([]);
        const [rows] = await db.promise().query(
...
router.get('/dashboard/appointments-by-date', authenticateToken, requireLab, async (req, res) => {
    try {
        const labId = await getCurrentLabId(req.user.id);
        if (!labId) return res.json({});
        const [rows] = await db.promise().query(
...
router.get('/dashboard/history/:userId', authenticateToken, requireLab, async (req, res) => {
    try {
        const labId = await getCurrentLabId(req.user.id);
...
router.put('/dashboard/me', authenticateToken, requireLab, async (req, res) => {
    try {
        ...
        const { name, login_email, address, phone, contact_email, description, working_hours } = req.body;
        ...
        try {
            ...
            if (name || login_email) {
                ...
                if (login_email) { updates.push('email=?'); params.push(login_email); }
                params.push(req.user.id);
                await trx.query(`UPDATE users SET ${updates.join(', ')} WHERE id=?`, params);
            }
            await trx.query(
                'UPDATE laboratories SET address=?, phone=?, email=?, description=?, working_hours=? WHERE id=?',
                [address || null, phone || null, contact_email || null, description || null, working_hours || null, labId]
            );
            await trx.commit();
            trx.release();
...
router.get('/dashboard/my-analysis-types', authenticateToken, requireLab, async (req, res) => {
    try {
        const labId = await getCurrentLabId(req.user.id);
        if (!labId) return res.json([]);
        const [rows] = await db.promise().query('SELECT id, name, description, normal_range, unit, price FROM analysis_types WHERE laboratory_id=? ORDER BY name', [labId]);
        res.json(rows);
    } catch (err) {
        console.error(err);
...
router.post('/dashboard/my-analysis-types', authenticateToken, requireLab, async (req, res) => {
    try {
        const labId = await getCurrentLabId(req.user.id);
        ...
        const [r] = await db.promise().query(
            ...
            [name, description || null, normal_range || null, unit || null, price || null, labId]
        );
...
router.put('/dashboard/my-analysis-types/:id', authenticateToken, requireLab, async (req, res) => {
    try {
        const labId = await getCurrentLabId(req.user.id);
        const id = req.params.id;
        const { name, description, normal_range, unit, price } = req.body;
        ...
        await db.promise().query(
            ...
            [name, description || null, normal_range || null, unit || null, price || null, id, labId]
        );
        res.json({ success: true });
    } catch (err) {
        console.error(err);
...
router.delete('/dashboard/my-analysis-types/:id', authenticateToken, requireLab, async (req, res) => {
    try {
        const labId = await getCurrentLabId(req.user.id);
        const id = req.params.id;
        await db.promise().query('DELETE FROM analysis_types WHERE id=? AND laboratory_id=?', [id, labId]);
        res.json({ success: true });
    } catch (err) {
        console.error(err);
...
router.get('/dashboard/confirmed', authenticateToken, requireLab, async (req, res) => {
    try {
        const labId = await getCurrentLabId(req.user.id);
        if (!labId) return res.json([]);
        const [rows] = await db.promise().query(
...
router.post('/dashboard/mark-pending/:id', authenticateToken, requireLab, async (req, res) => {
    try {
        const labId = await getCurrentLabId(req.user.id);
        const id = req.params.id;
        await db.promise().query(
...
router.post('/', authenticateToken, isAdmin, async (req, res) => {
    ...
    try {
        ...
        const [labResult] = await trx.query(
            ...
            [userId, address || null, phone || null, contact_email || null, description || null, working_hours || null]
        );
        ...
        return res.status(500).json({ error: 'Failed to create laboratory', details: error.message });
    }
});
...
router.put('/:id', authenticateToken, isAdmin, async (req, res) => {
    ...
    try {
        ...
        const [rows] = await trx.query('SELECT user_id FROM laboratories WHERE id = ?', [labId]);
        if (rows.length === 0) {
            await trx.rollback();
            trx.release();
        ...
        return res.status(500).json({ error: 'Failed to update laboratory', details: error.message });
    }
});
...
router.delete('/:id', authenticateToken, isAdmin, async (req, res) => {
    ...
    try {
        ...
        return res.status(500).json({ error: 'Failed to delete laboratory', details: error.message });
    }
});
...
router.post('/analysis-types', authenticateToken, isAdmin, async (req, res) => {
    ...
    try {
        const [result] = await db.promise().query(
            'INSERT INTO analysis_types (name, description, normal_range, unit, price, laboratory_id) VALUES (?, ?, ?, ?, ?, ?)',
            [name, description || null, normal_range || null, unit || null, price || null, laboratory_id]
        );
        return res.status(201).json({ id: result.insertId });
    } catch (error) {
        console.error('Create analysis type failed:', error);
        return res.status(500).json({ error: 'Failed to create analysis type', details: error.message });
    }
});

// Labs minimal list for dropdown (id + name)
router.get('/_dropdown/minimal', authenticateToken, isAdmin, async (req, res) => {
    try {
        const [rows] = await db.promise().query(
            `SELECT l.id, u.name
             FROM laboratories l
    ...
    try {
        ...
        for (let hour = startHour; hour < endHour; hour++) {
            for (let minute = 0; minute < 60; minute += 30) {
                ...
                const isAvailable = await Laboratory.isTimeSlotAvailableForDisplay(labId, localSlot);
