...
router.get('/doctor/dashboard', authenticateToken, requireDoctor, async (req, res) => {
    try {
...
router.get('/doctor/stats', authenticateToken, requireDoctor, async (req, res) => {
    try {
        const doctorId = await getCurrentDoctorId(req.user.id);
        if (!doctorId) return res.json({ total_therapies: 0, active_therapies: 0, completed_therapies: 0, cancelled_therapies: 0 });
...
router.get('/doctor/upcoming-followups', authenticateToken, requireDoctor, async (req, res) => {
    try {
        const doctorId = await getCurrentDoctorId(req.user.id);
        if (!doctorId) return res.json([]);
...
router.post('/doctor/create', authenticateToken, requireDoctor, async (req, res) => {
    try {
        ...
        if (!appointment_id || !patient_id || !therapy_text) {
            return res.status(400).json({ error: 'appointment_id, patient_id, and therapy_text are required' });
        }
        ...
        res.status(201).json({ id: therapyId, message: 'Therapy created successfully' });
    } catch (error) {
        console.error(error);
        res.status(500).json({ error: 'Internal server error' });
    }
...
router.put('/doctor/:id', authenticateToken, requireDoctor, async (req, res) => {
    try {
        const doctorId = await getCurrentDoctorId(req.user.id);
        const therapyId = req.params.id;
...
router.delete('/doctor/:id', authenticateToken, requireDoctor, async (req, res) => {
    try {
        const doctorId = await getCurrentDoctorId(req.user.id);
        const therapyId = req.params.id;
...
router.get('/patient/upcoming-followups', authenticateToken, async (req, res) => {
    try {
        if (req.user.role !== 'user') {
            return res.status(403).json({ error: 'Patient access required' });
        }
...
router.get('/patient/:id', authenticateToken, async (req, res) => {
    try {
        ...
        const therapyId = req.params.id;
        const [rows] = await db.promise().query(
            'SELECT * FROM therapies WHERE id=? AND patient_id=?',
            [therapyId, req.user.id]
        );
