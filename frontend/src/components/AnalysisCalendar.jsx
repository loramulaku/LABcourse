...
const AnalysisCalendar = ({ labId, selectedDate, onDateSelect, onTimeSelect }) => {
  ...
  const checkDateStatus = useCallback(async (date) => {
    try {
      const response = await fetch(`${API_URL}/api/laboratories/${labId}/date-status/${date}`);
      if (response.ok) {
  ...
  const loadAvailableSlots = useCallback(async (date) => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/laboratories/${labId}/available-slots/${date}`);
      if (response.ok) {
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-6">
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-xl font-semibold text-gray-800">Select Date & Time</h3>
      ...
      {selectedDate && (
        <div className="border-t pt-6">
          ...
          {fullyBookedDates.has(selectedDate) && (
            <div className="mt-4 p-3 bg-red-50 border border-red-200 rounded-lg">
              <p className="text-red-600 text-sm font-medium">
                ⚠️ All time slots for this date are fully booked. Please choose another date.
              </p>
            </div>
          )}
        </div>
