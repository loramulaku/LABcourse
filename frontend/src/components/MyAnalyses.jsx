  ...
  const downloadPDF = async (analysisId) => {
    try {
      ...
      const response = await fetch(
        `${API_URL}/api/patient-analyses/download-result/${analysisId}`,
        {
          headers: {
  ...
  useEffect(() => {
    const fetchMyAnalyses = async () => {
      try {
        setLoading(true);
        const response = await fetch(
          `${API_URL}/api/patient-analyses/my-analyses`,
          {
            credentials: "include",
            headers: {
      ...
      {analyses.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-gray-500 mb-4">
            You haven't requested any laboratory analyses yet.
          </p>
          <a
            href="/laboratories"
        ...
        <div className="grid gap-6">
          {analyses.map((analysis) => (
            <div
              key={analysis.id}
              className="bg-white rounded-lg shadow-md p-6 border-l-4 border-sky-500"
            >
              <div className="flex justify-between items-start mb-4">
                <div className="flex-1">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                      ...
                      <p className="font-medium">
                        {formatLocalDateTime(analysis.appointment_date)}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`px-3 py-1 rounded-full text-sm font-medium border ${getStatusColor(analysis.status)}`}
                    >
              ...
              {analysis.status === "completed" && (
                <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded-lg">
                  ...
                  {/* Text Results */}
                  {analysis.result && (
                    <div className="text-green-700 whitespace-pre-wrap mb-3">
                      {analysis.result}
                    </div>
                  )}
                  ...
                  {analysis.result_note &&
                    analysis.result_note !== "READY_FOR_RESULT_UPLOAD" && (
                  ...
                  {analysis.completion_date && (
                    <p className="text-sm text-green-600 mt-2">
                      Completed on:{" "}
                      {formatLocalDateTime(analysis.completion_date, {
                        includeSeconds: true,
                      })}
                    </p>
                  )}
              ...
              {(analysis.status === "pending_result" ||
              ...
              <div className="mt-4 pt-4 border-t border-gray-200">
                <p className="text-sm text-gray-500">
                  Request ID: #{analysis.id} • Submitted:{" "}
