...
export default function LabCalendarDate() {
  ...
  const unconfirmedRequests = appointments.filter(
    (apt) => apt.status === "unconfirmed",
  );
  const confirmedAppointments = appointments.filter(
    (apt) => apt.status === "pending_result",
  );
  ...
  const loadAppointments = async () => {
    try {
      const data = await apiFetch(
        `${API_URL}/api/laboratories/dashboard/appointments-by-date/${date}`,
      );
  ...
  const updateStatus = async (id, status) => {
    try {
      await apiFetch(
        `${API_URL}/api/laboratories/dashboard/update-status/${id}`,
        {
          method: "POST",
  ...
  const handleUploadResult = async () => {
    ...
    try {
      ...
      await apiFetch(
        `${API_URL}/api/laboratories/dashboard/upload-result/${selectedAppointment.id}`,
        {
          method: "POST",
          body: formData,
        },
  ...
  const getStatusBadge = (status) => {
    ...
    return (
      <span
        className={`px-3 py-1 rounded-full text-xs font-bold border-2 ${config.color}`}
      >
        {config.label}
      </span>
    );
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex flex-col space-y-2 ml-6">
                    <button
                      onClick={() =>
                        updateStatus(appointment.id, "pending_result")
                      }
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 mb-6">
          <div className="p-6 border-b border-gray-200">
            ...
            <p className="text-sm text-gray-600 mt-1">
              Confirmed appointments - use Pending Result page for result upload
            </p>
          </div>
          <div className="divide-y divide-gray-200">
