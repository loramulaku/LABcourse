            ...
            <div className="space-y-3">
              {items.map((it) => (
                <div key={it.id} className="rounded-lg border border-green-200 bg-white p-3">
                  ...
                  {it.result_note && it.result_note !== 'READY_FOR_RESULT_UPLOAD' && (
                    <div className="mt-2 text-green-700">Result Note: {it.result_note}</div>
                  )}
                  {it.result_pdf_path && (
                    <PdfPreview url={`${API_URL}/${it.result_pdf_path.replace(/^\/?/, '')}`} />
                  )}
                </div>
              ))}
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="w-full max-w-5xl h-[85vh] bg-white rounded-xl shadow-2xl overflow-hidden border-2 border-green-300">
            <div className="flex items-center justify-between p-4 border-b border-green-200 bg-gradient-to-r from-green-50 to-green-100">
              <div className="text-lg font-bold text-green-800">PDF Result Preview</div>
              <button onClick={() => setOpen(false)} className="rounded-lg px-4 py-2 text-sm bg-green-600 hover:bg-green-700 text-white font-medium transition-all shadow-sm">Close</button>
            </div>
