        ...
        <div className="bg-white rounded-lg shadow-inner border border-green-200 overflow-hidden">
          <FullCalendar
            ...
            dayCellContent={(arg) => {
              ...
              const hasEvents = events.some(event => event.start === arg.date.toISOString().split('T')[0]);
              return (
                <div className={`p-1 ${hasEvents ? 'bg-green-50' : ''} hover:bg-green-100 transition-colors rounded`}>
                  {arg.dayNumberText}
                </div>
              );
            }}
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/70 p-4">
          <div className="w-full max-w-lg rounded-2xl bg-gradient-to-br from-green-50 to-white p-6 shadow-2xl border-2 border-green-400">
            <div className="mb-4 flex items-center justify-between">
              ...
              <button onClick={() => setSelected(null)} className="rounded-lg px-4 py-2 text-sm bg-green-600 hover:bg-green-700 text-white font-medium transition-all shadow-lg hover:shadow-xl">Close</button>
            </div>
            <div className="space-y-3 text-sm max-h-96 overflow-y-auto">
              {(selected.items || []).map(item => (
                <div key={item.id} className="rounded-xl border-2 border-green-300 bg-gradient-to-r from-green-100 to-green-50 p-4 shadow-md hover:shadow-lg transition-all">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <div className="text-green-700 text-xs mt-1">{item.user_email} {item.user_phone ? `• ${item.user_phone}` : ''}</div>
                    </div>
                  ...
                  {item.notes && (
                    <div className="mt-3 bg-white p-3 rounded-lg border border-green-200">
                      <div className="flex items-start">
                        <svg className="w-4 h-4 mr-2 text-green-600 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                        </svg>
                        <div>
                          <div className="text-xs text-green-600 font-medium mb-1">Notes:</div>
