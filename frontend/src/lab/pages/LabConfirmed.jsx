    ...
    const load = async () => {
      try {
        const data = await apiFetch(
          `${API_URL}/api/laboratories/dashboard/confirmed`,
        );
        setPatients(data);
      } catch (e) {
        console.error(e);
          ...
          >
            <div className="flex-1">
              ...
              <p className="text-green-600">
                {new Date(p.appointment_date).toLocaleString()}
              </p>
            ...
            <button
              onClick={() => markPendingResult(p.id)}
              className="rounded-lg border border-orange-300 px-4 py-2 hover:bg-orange-50 text-orange-600 font-medium transition-all"
            >
              Move to Pending Result
            </button>
          </div>
        ))}
      </div>
