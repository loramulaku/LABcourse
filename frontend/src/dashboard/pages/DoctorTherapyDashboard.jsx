...
const DoctorTherapyDashboard = () => {
  ...
  const fetchTherapiesByStatus = async (status) => {
    try {
      const response = await fetch(`${API_URL}/api/therapies/doctor/status/${status}`, {
        headers: { Authorization: `Bearer ${getAccessToken()}` },
        credentials: 'include',
      });
  ...
  const updateTherapyStatus = async (therapyId, newStatus) => {
    try {
      const response = await fetch(`${API_URL}/api/therapies/doctor/${therapyId}/status`, {
        method: 'PATCH',
        headers: {
  ...
  return (
    <>
      <PageMeta title="Therapy Management" description="Manage patient therapies" />
            ...
            <div className="bg-white p-6 rounded-lg shadow-md">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  ...
                  <p className="text-2xl font-bold text-gray-900">{stats.pending_therapies || 0}</p>
                </div>
              </div>
            </div>
              ...
              <table className="w-full">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Therapy Type</th>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredTherapies.map((therapy) => (
                    <tr key={therapy.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">{therapy.therapy_type || 'General Therapy'}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">
                          {therapy.start_date ? formatDate(therapy.start_date) : 'Not set'}
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getPriorityColor(therapy.priority)}`}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div className="flex space-x-2">
                          <button
                            onClick={() => navigate(`/dashboard/therapy/edit/${therapy.id}`)}
                          ...
                          {therapy.status === 'draft' && (
                            <button
                              onClick={() => updateTherapyStatus(therapy.id, 'pending')}
                          ...
                          {therapy.status === 'pending' && (
                            <button
                              onClick={() => updateTherapyStatus(therapy.id, 'confirmed')}
                              className="text-green-600 hover:text-green-900"
                            >
                              Confirm
                            </button>
